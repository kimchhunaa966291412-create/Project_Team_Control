(function() {
            ...
            /* ============================================================
               ...
               6. Commit this file — every teammate who opens the site will
                  now see everyone else's edits live, no extra setup needed.
               If you skip this, the app still works exactly as before,
               saving only to each person's own browser (localStorage).
               ============================================================ */
            const FIREBASE_CONFIG = {
                apiKey: "AIzaSyBwwLSU8IiH8Z8Haby-wmAVTUeiSJFk3-k",
                authDomain: "team-famer-ai-control.firebaseapp.com",
                projectId: "team-famer-ai-control",
                storageBucket: "team-famer-ai-control.firebasestorage.app",
                messagingSenderId: "1077449992340",
                appId: "1:1077449992340:web:a36acb928dd9351ff22c81"
            };
            const CLOUD_COLLECTION = "khmercrop-project-control";
            const CLOUD_DOC_ID = "team-data";
            const CLOUD_SYNC_ENABLED = !!(FIREBASE_CONFIG.apiKey && FIREBASE_CONFIG.apiKey !== "YOUR_API_KEY");
            ...
            /* ============================================================
               CLOUD SYNC — real-time multi-user updates via Firestore
               ============================================================ */
            function setSyncStatus(state) {
                const dot = document.getElementById("kc-sync-dot");
                const label = document.getElementById("kc-sync-label");
                if (!dot || !label) return;
                const map = {
                    connecting: { color: "#f59e0b", text: "កំពុងភ្ជាប់ទៅក្រុម..." },
                    synced: { color: "#22c55e", text: "🟢 Sync ជាមួយក្រុមផ្សេងទៀត (Real-time)" },
                    offline: { color: "#ef4444", text: "⚠️ គ្មានអ៊ីនធឺណិត — កំពុងប្រើទិន្នន័យក្នុងឧបករណ៍" },
                    local: { color: "#94a3b8", text: "ទិន្នន័យក្នុងឧបករណ៍នេះតែប៉ុណ្ណោះ (មិនទាន់ភ្ជាប់ Sync)" }
                };
                const s = map[state] || map.local;
                dot.style.background = s.color;
                dot.style.animation = (state === "connecting") ? "pulse-dot 1s ease-in-out infinite" :
                    "pulse-dot 2s ease-in-out infinite";
                label.textContent = s.text;
            }

            function initCloudSync() {
                if (!CLOUD_SYNC_ENABLED) { setSyncStatus("local"); return; }
                ...
                try {
                    ...
                    firebase.auth().signInAnonymously().then(function() {
                        ...
                        cloudDocRef.get().then(function(snap) {
                            if (snap.exists) {
                                data = withDefaults(snap.data());
                                render();
                            } else {
                                cloudDocRef.set(data).catch(function(e) {
                                    console.error("Cloud init write failed", e);
                                });
                            }
                            cloudReady = true;
                            subscribeCloud();
            ...
            function subscribeCloud() {
                ...
                cloudDocRef.onSnapshot(function(snap) {
                    ...
                    if (isEditing) {
                        ...
                        showSyncPullBanner();
                        toast("🔄 សមាជិកម្នាក់ទៀតទើបតែកែប្រែ — ចុច Ctrl+S ឬប៊ូតុងខាងលើដើម្បីទាញយកបច្ចុប្បន្នភាព",
                            4500);
                    } else {
                        data = withDefaults(incoming);
                        render();
                    }
                    setSyncStatus("synced");
                }, function(err) {
            ...
            function applyPendingRemoteData() {
                if (!pendingRemoteData) return;
                data = withDefaults(pendingRemoteData);
                pendingRemoteData = null;
                hideSyncPullBanner();
                render();
                toast("🔄 បានទាញយកបច្ចុប្បន្នភាពពីក្រុម");
            }

            const kcSyncPullBtn = document.getElementById("kc-sync-pull-btn");
            if (kcSyncPullBtn) kcSyncPullBtn.addEventListener("click", applyPendingRemoteData);

            function showSyncPullBanner() {
                const banner = document.getElementById("kc-sync-pull-banner");
                if (banner) banner.classList.remove("hidden");
            }

            function hideSyncPullBanner() {
                const banner = document.getElementById("kc-sync-pull-banner");
                if (banner) banner.classList.add("hidden");
            }

            function pushToCloud() {
                if (!CLOUD_SYNC_ENABLED || !cloudDocRef || !cloudReady) return;
                clearTimeout(cloudSaveTimer);
                cloudSaveTimer = setTimeout(function() {
                    cloudDocRef.set(data).catch(function(err) {
            ...
            window.addEventListener("online", function() {
                if (CLOUD_SYNC_ENABLED) setSyncStatus(cloudReady ? "synced" : "connecting");
            });
            window.addEventListener("offline", function() {
                if (CLOUD_SYNC_ENABLED) setSyncStatus("offline");
            });
